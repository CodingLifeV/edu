...
`);const _={},$=[],w=typeof g=="function"?g:r=>(g||"").replace(/:key/g,m(e)).replace(/:name/g,m(r));for(const r in f){if(o){const u=`${r}${D(o.replace(/:key/g,m(e)))}`,h=await T(t,{path:u,frontmatter:{...d(r),blog:{type:"category",key:e},layout:c}}),s=t.pages.findIndex(({path:E})=>E===u);s===-1?t.pages.push(h):t.pages[s].key!==h.key&&(t.pages.splice(s,1,h),l&&n.warn(`Overiding existed path ${u}`)),$.push(h.key),_[r]={path:h.path,map:{}}}else _[r]={path:"",map:{}};const{map:b}=_[r],P={};for(const u of f[r]){const h=i(u);for(const s of h){if(!b[s]){const E=w(s);if(E){const U=`${r}${D(E)}`,R=await T(t,{path:`${r}${D(E)}`,frontmatter:{...y(s,r),blog:{type:"category",name:s,key:e},layout:k}}),I=t.pages.findIndex(({path:N})=>N===U);I===-1?t.pages.push(R):t.pages[I].key!==R.key&&(t.pages.splice(I,1,R),l&&n.warn(`Overiding existed path ${U}`)),$.push(R.key),b[s]={path:R.path,keys:[]}}else b[s]={path:"",keys:[]};P[s]=[]}P[s].push(u)}}for(const u in P)b[u].keys=P[u].sort(a).map(({key:h})=>h);if(t.env.isDebug){let u=`Route ${r} in ${e} cateogry:
...
`);for(const p in f){const _=f[p].filter(a).sort(i).map(({key:$})=>$);if(o){const $=`${p}${D(m(o.replace(/:key/g,e)))}`,w=await T(t,{path:$,frontmatter:{...d(p),blog:{type:"type",key:e},layout:c}}),r=t.pages.findIndex(({path:b})=>b===$);r===-1?t.pages.push(w):t.pages[r].key!==w.key&&(t.pages.splice(r,1,w),l&&n.warn(`Overiding existed path ${$}`)),y.push(w.key),k[p]={path:w.path,keys:_},t.env.isDebug&&n.info(`Route ${p} in ${e} type: path: ${w.path}; items: ${_.length}
