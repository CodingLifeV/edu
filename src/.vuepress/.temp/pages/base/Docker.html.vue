...
<p>试想有这样一个案例：</p>
<p>我们在公司研发了一个 APP，程序员在自己的电脑上搭建了一套环境开始写代码，写完之后如释重负，把代码交给了测试人员测试，测试人员又开始从头到尾搭建环境之后在进行测试。测试完成之后，测试人员又将代码交给运维人员，运维在从头到尾搭建环境，最终才可上线。然而，在反复搭建环境的过程中，往往会出现一系列“在别人的电脑上可以运行的，我的却不行”诸如此类的问题，使用 Docker 容器技术可以完美解决以上问题</p>
</div>
<p>Docker 是一个虚拟环境容器，可以将你的开发环境、代码、配置文件等一并打包到这个容器中，并发布和应用到任意平台中。通过 Docker 可以轻松屏蔽环境差异，程序员只需要将自己的代码、开发环境、配置文件等内容使用 Docker 打包到一个容器中，测试和运维人员无需在进行环境搭建，拿来即用，真正做到“build once, run everywhere”</p>
<p>Docker 使用客户端-服务器架构, Docker 客户端与 Docker daemon （Docker 守护进程）对话，后者负责构建、运行和分发 Docker 容器。Docker 客户端和 Docker daemon 可以在同一系统上运行，或者也可以将 Docker 客户端连接到远程 Docker daemon。Docker 中有几个重要的概念：</p>
<ol>
<li>
<p>镜像（<code v-pre>Image</code>）：类似于虚拟机中的镜像，是一个包含有文件系统的面向 Docker 引擎的只读模板。可以简单理解为一个镜像即是一个可执行程序。</p>
</li>
<li>
<p>容器（<code v-pre>Container</code>）：类似于运行起来的进程，可以将其看作一个极简的 Linux 系统环境以及运行在其中的应用程序，有了“可执行程序” image 之后，将其加载到内存开始执行，即是所谓的 container。Docker 引擎利用容器来运行、隔离各个应用。容器是镜像创建的应用实例，可以创建、启动、停止、删除容器，各个容器之间是是相互隔离的，互不影响。</p>
</li>
<li>
<p>仓库（<code v-pre>Registry</code>）：类似于远程代码仓库，这里是镜像的仓库，是 Docker 用来集中存放镜像文件的地方。</p>
</li>
<li>
...
<h3 id="docker-安装与原理再探" tabindex="-1"><a class="header-anchor" href="#docker-安装与原理再探" aria-hidden="true">#</a> Docker 安装与原理再探</h3>
<p>Docker可以安装在 Windows、Linux、Mac 等各个平台上，以 CentOS 为例，要在 CentOS 上开始使用 Docker 引擎，请确保满足<strong>先决条件</strong>：CentOS 版本要求 CentOS 7 或者 CentOS 8，然后在安装 Docker，更多安装方式可移步官方参考<a href="https://docs.docker.com/engine/install/" target="_blank" rel="noopener noreferrer">Install Docker Engine | Docker Documentation<ExternalLinkIcon/></a>，Linux 终端 CentOS 下安装 Docker 步骤如下：</p>
<ol>
<li>
<p>确定 CentOS 版本，要求CentOS 7 或者 CentOS 8</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@VM-24-11-centos ~<span class="token punctuation">]</span><span class="token comment"># cat /etc/os-release</span>
...
<li>
<p>卸载旧版本 Docker</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">sudo</span> yum remove <span class="token function">docker</span> <span class="token punctuation">\</span>
...
<li>
<p>安装<code v-pre>yum-utils</code>软件包</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">sudo</span> yum <span class="token function">install</span> <span class="token parameter variable">-y</span> yum-utils
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>设置镜像的仓库，推荐使用国内阿里云仓库</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 国内,推荐使用</span>
...
<li>
<p>安装 Docker 社区版 <code v-pre>docker-ce</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">sudo</span> yum <span class="token function">install</span> docker-ce docker-ce-cli containerd.io
...
<p>如果你想安装具体版本的 Docker ，需要执行以下命令：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><code v-pre>&lt;VERSION_STRING&gt;</code>为版本号，例如 <code v-pre>docker-ce-18.09.1</code></p>
</div>
</li>
<li>
<p>测试是否安装成功</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@VM-24-11-centos ~<span class="token punctuation">]</span><span class="token comment"># sudo docker version</span>
...
<ol>
<li>
<p>客户端发起 <code v-pre>docker run</code> 命令向 <code v-pre>Docker</code> 服务器端</p>
</li>
<li>
<p>服务器端守护进程查找本地系统是否有 <strong>hello-world</strong> 镜像存在</p>
</li>
<li>
<p>服务器端守护进程向 Docker 远程库（Docker Hub）寻找 <strong>hello-world</strong>  镜像</p>
</li>
<li>
<p>服务器端守护进程将远程 <strong>hello-world</strong>  镜像下载到本地系统</p>
</li>
<li>
...
<p>Docker 相关命令参考<a href="https://docs.docker.com/engine/api/" target="_blank" rel="noopener noreferrer">Docker CLI<ExternalLinkIcon/></a></p>
<p>当我们安装完 Docker 引擎之后，就可以对镜像 Image 进行基本的操作了。比如我们从<a href="https://hub.docker.com/" target="_blank" rel="noopener noreferrer">官方注册服务器<ExternalLinkIcon/></a>的仓库中 Docker Hub 下载 CentOS 的镜像，流程如下：</p>
<ol>
<li>
...
<li>
<p>删除镜像</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># image_id 镜像 ID</span>
...
<h3 id="docker-中关于容器的基本操作" tabindex="-1"><a class="header-anchor" href="#docker-中关于容器的基本操作" aria-hidden="true">#</a> Docker 中关于容器的基本操作</h3>
<p>我们已经提到过，镜像 Image 相当于一个可执行程序，是一个只读模板，而容器 Container 类似于运行起来的进程。有了 “可执行程序”  image 之后，将其加载到内存开始执行，即是所谓的 container。一个容器可以创建、启动、停止、删除，容器的基本操作主要包含如下</p>
<ol>
<li>
<p>查看当前正在运行的容器</p>
...
<li>
<p>新建容器并运行（流程和 <a href="#_link_docker1">hello-world</a> 同理）</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-it</span> centos:latest /bin/bash
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="custom-container tip">
...
<li>
<p>启动容器或者重启容器</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 启动容器</span>
...
<li>
<p>删除容器</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 删除指定容器, 不能删除正在运行的容器</span>
...
<h3 id="dockerfile—新建自有镜像" tabindex="-1"><a class="header-anchor" href="#dockerfile—新建自有镜像" aria-hidden="true">#</a> Dockerfile—新建自有镜像</h3>
<p>我们已经了解了关于镜像和容器的基本操作，那么如何将自己的项目构建成 Docker 镜像以方便其它人员使用呢？Docker 中提供了两种方式构建自有镜像：<strong>修改已有镜像和使用 Dockerfile 创建镜像</strong></p>
<h4 id="使用-dockerfile-创建镜像" tabindex="-1"><a class="header-anchor" href="#使用-dockerfile-创建镜像" aria-hidden="true">#</a> 使用 Dockerfile 创建镜像</h4>
<p>Dockerfile 是一个用来构建镜像 Image 的文本文件，Docker 可以通过读取 Dockerfile 中的指令来自动构建镜像，使用 <code v-pre>docker build</code> 命令，我们可以创建一个连续执行多个命令行指令的自有镜像。Dockerfile 主要包含以下指令：</p>
<table>
<thead>
<tr>
...
<td style="text-align:left"><code v-pre>FROM &lt;image&gt;</code><br/><code v-pre>FROM &lt;image&gt;:&lt;tag&gt;</code></td>
<td>创建镜像使用的基础镜像</td>
</tr>
<tr>
<td style="text-align:left"><strong>MAINTAINER</strong></td>
<td style="text-align:left"><code v-pre>MAINTAINER &lt;name&gt;</code></td>
<td>镜像作者信息</td>
</tr>
<tr>
<td style="text-align:left"><strong>RUN</strong></td>
<td style="text-align:left"><code v-pre>RUN &lt;command&gt;</code></td>
<td>编译镜像运行的命令</td>
</tr>
<tr>
<td style="text-align:left"><strong>EXPOSE</strong></td>
<td style="text-align:left"><code v-pre>EXPOSE port</code></td>
<td>容器暴露端口号</td>
</tr>
<tr>
...
<td style="text-align:left"><code v-pre>WORKDIR /path/to/workdir</code></td>
<td>指定容器的工作目录</td>
</tr>
<tr>
<td style="text-align:left"><strong>VLOUME</strong></td>
<td style="text-align:left"><code v-pre>VOLUME [&quot;/path/to/directory&quot;]</code></td>
<td>设置容器卷</td>
</tr>
<tr>
<td style="text-align:left"><strong>ADD</strong></td>
<td style="text-align:left"><code v-pre>ADD &lt;src&gt; &lt;dest&gt;</code></td>
<td>把本机的文件复制到镜像中，<br/>src 是压缩文件会自动解压出来</td>
</tr>
<tr>
<td style="text-align:left"><strong>COPY</strong></td>
<td style="text-align:left"><code v-pre>COPY &lt;src&gt; &lt;dest&gt;</code></td>
<td>与 ADD 指令类似，<br/>COPY 的是压缩包不会自动解压</td>
</tr>
<tr>
<td style="text-align:left"><strong>CMD</strong></td>
<td style="text-align:left"><code v-pre>CMD &lt;command&gt;</code></td>
<td>容器启动时要运行的命令，只有在执行<br/><code v-pre>docker run</code>或者<code v-pre>docker start</code>命令时才<br/>会运行，如果一个 Dockerfile有多条 CMD <br/> 指令，那么只有文件最后一行的CMD指令 <br/>才会生效</td>
</tr>
<tr>
<td style="text-align:left"><strong>ENTRYPOINT</strong></td>
<td style="text-align:left"><code v-pre>ENTRYPOINT &lt;command&gt;</code></td>
<td>与 CMD 类似，如果一个 Dockerfile 有多<br/>条 CMD 指令，可以追加命令</td>
</tr>
</tbody>
</table>
<p>例如我们在 Linux 环境下构建一个 CentOS 镜像 mycentos01，并且该镜像包含一个 tomcat，步骤如下：</p>
<ol>
<li>
<p>准备安装所需文件：tomcat、JDK压缩包，创建 dockerfile 文件，放在同级目录下</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@VM-24-11-centos mydocker<span class="token punctuation">]</span><span class="token comment"># cd /opt/mydocker/</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>dockerfile 文件内容（按 <code v-pre>i</code> 编辑 ，按 <code v-pre>ESC</code> 退出编辑，按<code v-pre>Shift</code> + <code v-pre>:wq</code> 保存退出）：</p>
<div class="language-docker line-numbers-mode" data-ext="docker"><pre v-pre class="language-docker"><code><span class="token instruction"><span class="token keyword">FROM</span> centos:latest</span>
...
<li>
<p>运行启动的镜像</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-d</span> <span class="token parameter variable">-p</span> <span class="token number">5566</span>:8080 <span class="token parameter variable">--name</span> mytomcat01 codinglife/mytomcat01:1.0 
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>参数<code v-pre>-p</code>表示指定端口映射，格式为：<code v-pre>主机(宿主)端口 : 容器端口</code> （我们已经知道，可以将 Docker 容器看作一个极简的 Linux 系统环境以及运行在其中的应用程序，因此需要开启端口映射来访问容器中的应用程序）</p>
</li>
</ol>
<h4 id="修改已有镜像" tabindex="-1"><a class="header-anchor" href="#修改已有镜像" aria-hidden="true">#</a> 修改已有镜像</h4>
<p>当然，除了使用 dockerfile 文件构建自有镜像之外，还可以修改原有的镜像，之后将修改后的镜像提交为一个新的镜像。例如在一个 centos 镜像中安装git、vim方法如下：</p>
<ol>
<li>
<p>运行一个 centos 容器，使用<code v-pre>-it</code>参数进入其伪终端</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@VM-24-11-centos /<span class="token punctuation">]</span><span class="token comment"># docker run -it centos /bin/bash</span>
...
<li>
<p>在伪终端下安装 git 、vim</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@7c072f2eea68 /<span class="token punctuation">]</span><span class="token comment"># yum -y install git</span>
...
<p class="custom-container-title">Tips</p>
<p>1、官方的 centos 镜像伪终端类似一个极简的 Linux 系统，因此不包含许多命令，需要我们手动安装</p>
<p>2、以上命令也可以写在 dockerfile 文件中，使用 RUN 指令运行：</p>
<div class="language-docker line-numbers-mode" data-ext="docker"><pre v-pre class="language-docker"><code><span class="token instruction"><span class="token keyword">RUN</span> yum -y install git</span>
<span class="token instruction"><span class="token keyword">RUN</span> yum -y install vim</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>3、如果你的命令运行一直失败，可能是由于镜像源访问不到，可以在伪终端下重新安装阿里云镜像源，安装方式参考<a href="https://developer.aliyun.com/mirror/centos?spm=a2c6h.13651102.0.0.3e221b11WUg2mF" target="_blank" rel="noopener noreferrer">centos镜像-centos下载地址-centos安装教程-阿里巴巴开源镜像站 (aliyun.com)<ExternalLinkIcon/></a></p>
</div>
</li>
<li>
...
<p>Docker容器产生的数据，如果不通过<code v-pre>docker commit</code>生成新的镜像，使得数据做为镜像的一部分保存下来，那么当容器删除后，数据自然也就没有了，为了能保存数据在 Docker 中，Docker 使用数据卷（Data Volumes）、数据卷容器（Data Volumes Dontainers）来确保数据的持久化和同步。</p>
<div class="custom-container tip">
<p class="custom-container-title">Tips</p>
<p>数据卷就是数据(一个文件或者文件夹)，本质上对应的是宿主机的文件目录</p>
</div>
<p>参考网址：</p>
<ul>
<li><a href="http://www.lining0806.com/%E6%95%B0%E6%8D%AE%E5%8D%B7%E4%B8%8E%E6%95%B0%E6%8D%AE%E5%8D%B7%E5%AE%B9%E5%99%A8/" target="_blank" rel="noopener noreferrer">Docker数据卷与数据卷容器<ExternalLinkIcon/></a></li>
<li><a href="https://developer.aliyun.com/article/622327?spm=a2c6h.13813017.content3.1.38d56878JxV9M9" target="_blank" rel="noopener noreferrer">Docker系列教程14-Docker数据持久化<ExternalLinkIcon/></a></li>
<li><a href="https://segmentfault.com/a/1190000022274697" target="_blank" rel="noopener noreferrer">docker 入门到实战（五）docker容器数据卷<ExternalLinkIcon/></a></li>
<li><a href="https://jiajially.gitbooks.io/dockerguide/content/chapter_fastlearn/docker_run/--volumes-from.html" target="_blank" rel="noopener noreferrer">数据卷容器（--volumes-from） | Docker-guide (gitbooks.io)<ExternalLinkIcon/></a></li>
</ul>
<h4 id="数据卷" tabindex="-1"><a class="header-anchor" href="#数据卷" aria-hidden="true">#</a> 数据卷</h4>
<p>使用数据卷持久化一个 CentOS 容器数据操作如下：</p>
<ol>
<li>
<p>使用 <code v-pre>-v</code> 参数运行一个容器</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-it</span> <span class="token parameter variable">-v</span> /opt/mydocker/myvolume:/opt/myvolume centos /bin/bash
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="custom-container tip">
<p class="custom-container-title">Tips</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 具名挂载格式: -v 卷名:容器内路径</span>
...
<li>
<p>构建自有镜像时，设置数据卷</p>
<p>我们可以在构建自有镜像时为容器指定数据卷，例如构建一个简单的 CentOS 镜像，dockerfile 文件内容如下：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># dockerfile文件内容</span>
FROM centos
...
<li>
<p>删除数据卷</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> <span class="token function">rm</span> <span class="token parameter variable">-v</span> 容器ID
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
</ol>
<h4 id="数据卷容器" tabindex="-1"><a class="header-anchor" href="#数据卷容器" aria-hidden="true">#</a> 数据卷容器</h4>
<p>通过数据卷可以实现容器内目录和宿主机目录之间数据同步，如果你有一些持续更新的数据需要在容器之间共享，最好创建数据卷容器。 数据卷容器，其实就是一个正常的容器，专门用来提供数据卷供其它容器挂载。数据卷容器的实现要求我们使用参数<code v-pre>--volumes-from</code>来实现一个继承关系，使用数据卷容器流程如下：</p>
<ol>
<li>
<p>构建一个数据卷容器（父容器），并为该容器添加一些数据（这里添加一个readme.txt文件）</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-it</span> <span class="token parameter variable">--name</span> father <span class="token parameter variable">-v</span> /opt/myvolumn centos /bin/bash
<span class="token builtin class-name">cd</span> /opt/myvolumn/
<span class="token function">touch</span> readme.txt
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>构建挂载步骤 1 数据卷的容器（创建子容器继承父容器）</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-it</span> <span class="token parameter variable">--name</span> son1 --volumes-from father centos
...
<h3 id="将本地镜像推送到远程库-docker-hub" tabindex="-1"><a class="header-anchor" href="#将本地镜像推送到远程库-docker-hub" aria-hidden="true">#</a> 将本地镜像推送到远程库 Docker Hub</h3>
<p>当我们学会如何构建自有镜像之后，便可以将构建的镜像推送到 Docker Hub 上供其它人员使用了，流程如下：</p>
<ol>
<li>
<p>访问<a href="https://hub.docker.com/" target="_blank" rel="noopener noreferrer">Docker Hub<ExternalLinkIcon/></a>，如果没有账号，需要先注册一个</p>
</li>
<li>
